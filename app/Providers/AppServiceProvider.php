...
use App\Reports\TeacherReportCardImplementation;
...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        QueryBuilder::macro('whereUnaccent', function ($column, $value) {
            $this->whereRaw(
                'unaccent(' . $column . ') ilike unaccent(\'%\' || ? || \'%\')',
                [$value]
            );
        });
    }
    ...
    public function register()
    {
        ...
        // StudentUnificationService
        $this->app->bind(StudentUnificationService::class, function () {
            return new StudentUnificationService(Auth::user());
        });

        // Registrar TeacherReportCard (UMA ÚNICA VEZ)
        $this->app->bind(
            TeacherReportCard::class,
            TeacherReportCardImplementation::class
        );
