...
return new class extends clsCadastro
{
    ...
    public function Gerar()
    {
        ...
        $this->ieducar_background_image     = $configuracoes['ieducar_background_image'] ?? '';
        $this->ieducar_background_image_url = $configuracoes['ieducar_background_image_url'] ?? '';
        $this->url_diario_professor         = $configuracoes['url_diario_professor'] ?? '';
        $this->url_whatsapp                 = $configuracoes['url_whatsapp'] ?? '';
        $this->mostrar_botoes_ajuda_login   = isset($configuracoes['mostrar_botoes_ajuda_login'])
            ? dbBool(val: $configuracoes['mostrar_botoes_ajuda_login'])
            : true;
        ...
        // ── Logo upload ─────────────────────────────────────────────────────
        $logoAtual = $this->ieducar_image;
        $logoPreviewHtml = $logoAtual
            ? "<div style='margin-top:8px;'><strong>Logo atual:</strong><br>
               <img id='preview_logo' src='{$logoAtual}' style='max-height:80px;max-width:200px;border:1px solid #ccc;border-radius:4px;margin-top:4px;' alt='Logo'><br>
               <a href='{$logoAtual}' target='_blank' style='font-size:12px;'>🔗 Ver imagem completa</a><br>
               <small style='color:#666;'>Dica: Para alterar, faça novo upload ou insira nova URL.</small></div>"
            : "<img id='preview_logo' src='' style='display:none;max-height:80px;max-width:200px;border:1px solid #ccc;border-radius:4px;margin-top:4px;'>";
        ...
        $this->campoRotulo(nome: 'lbl_logo_url', campo: 'URL da logo (opcional)', valor:
            '<input type="text" name="ieducar_image" id="ieducar_image" value="' . htmlspecialchars($this->ieducar_image ?? '') . '" size="80" maxlength="500" style="width:100%;" placeholder="https://...">');

        $this->campoRotulo(nome: 'lbl_logo_upload', campo: 'OU faça upload de uma imagem:', valor:
            '<label class="btn btn-default" style="cursor:pointer;">
                <span style="background:#28a745;color:white;padding:5px 12px;border-radius:4px;">Escolha um arquivo</span>
                <input type="file" accept="image/*" style="display:none;" onchange="previewImagem(this, \'preview_logo\', \'ieducar_image_upload\')">  
            </label>
            <input type="hidden" name="ieducar_image_upload" id="ieducar_image_upload" value="">');

        $this->campoRotulo(nome: 'lbl_logo_preview', campo: 'Preview da logo::', valor: $logoPreviewHtml);

        // ── Imagem de fundo do login ─────────────────────────────────────────
        $bgAtual = $this->ieducar_background_image ?? '';
        $bgUrl   = $this->ieducar_background_image_url ?? '';
        $bgSrc   = $bgUrl ?: $bgAtual;

        $bgPreviewHtml = $bgSrc
            ? "<img id='preview_bg' src='{$bgSrc}' style='max-height:120px;max-width:300px;border:1px solid #ccc;border-radius:4px;margin-top:4px;' alt='Background'>"
            : "<div id='preview_bg_box' style='border:2px dashed #ccc;border-radius:6px;padding:20px;text-align:center;color:#888;background:#f9f9f9;'>
               <input type='checkbox' disabled style='margin-bottom:8px;'><br>
               Nenhuma imagem de fundo configurada.<br>Será usada a imagem padrão.</div>
               <img id='preview_bg' src='' style='display:none;max-height:120px;max-width:300px;border:1px solid #ccc;border-radius:4px;margin-top:4px;'>";

        $this->campoRotulo(nome: 'lbl_bg_section', campo: 'Imagem de fundo da tela de login:', valor: '<hr style="border:none;border-top:1px solid #ddd;">');

        $this->campoRotulo(nome: 'lbl_bg_info', campo: '', valor:
            '<div style="background:#eef4ff;border-left:4px solid #4a90d9;padding:10px 14px;border-radius:4px;font-size:13px;">
             <strong>Instruções:</strong> A imagem aparecerá atrás da logo branca com 15% de opacidade.<br>
             <strong>Dica:</strong> Para substituir uma imagem, basta fazer um novo upload ou alterar a URL.</div>');

        $this->campoRotulo(nome: 'lbl_bg_url', campo: 'URL da imagem de fundo', valor:
            '<input type="text" name="ieducar_background_image_url" id="ieducar_background_image_url" value="' . htmlspecialchars($bgUrl) . '" size="80" maxlength="500" style="width:100%;" placeholder="Ex: https://exemplo.com/fundo.jpg ou https://exemplo.com/fundo.png">');

        $this->campoRotulo(nome: 'lbl_bg_upload', campo: 'OU faça upload de uma imagem:', valor:
            '<label class="btn btn-default" style="cursor:pointer;">
                <span style="background:#28a745;color:white;padding:5px 12px;border-radius:4px;">Escolha um arquivo</span>
                <input type="file" accept="image/jpeg,image/png,image/gif,image/webp" style="display:none;"
                       onchange="previewImagem(this, \'preview_bg\', \'ieducar_background_image\')">  
            </label>
            <input type="hidden" name="ieducar_background_image" id="ieducar_background_image" value="' . htmlspecialchars($bgAtual) . '">');

        $this->campoRotulo(nome: 'lbl_bg_fileinfo', campo: 'Informações::', valor:
            '<div style="background:#f5f5f5;border:1px solid #ddd;padding:10px 14px;border-radius:4px;font-size:12px;">
             ...
             <strong>Prioridade:</strong> URL &gt; Upload &gt; Imagem padrão</div>');

        $this->campoRotulo(nome: 'lbl_bg_preview', campo: 'Preview::', valor: $bgPreviewHtml);

        // ── Configurações dos Botões de Ajuda no Login ───────────────────────
        $this->campoRotulo(nome: 'lbl_ajuda_section', campo: 'Configurações dos Botões de Ajuda no Login:', valor: '<hr style="border:none;border-top:1px solid #ddd;">');

        $mostrarBotoes = isset($this->mostrar_botoes_ajuda_login) ? (bool)$this->mostrar_botoes_ajuda_login : true;
        $chkChecked    = $mostrarBotoes ? 'checked' : '';
        $this->campoRotulo(nome: 'lbl_mostrar_botoes', campo: 'Mostrar botões "Precisa de Ajuda?" na tela de login', valor:
            '<input type="checkbox" name="mostrar_botoes_ajuda_login" id="mostrar_botoes_ajuda_login" value="1" ' . $chkChecked . '>
             <input type="hidden" name="mostrar_botoes_ajuda_login_hidden" value="0">
             <br><small style="color:#666;">Se marcado, mostra os botões de ajuda na tela de login</small>');

        $this->campoRotulo(nome: 'lbl_url_diario', campo: 'URL do Diário do Professor', valor:
            '<input type="text" name="url_diario_professor" id="url_diario_professor" value="' . htmlspecialchars($this->url_diario_professor ?? '') . '" size="80" maxlength="500" style="width:100%;" placeholder="Ex: https://diario.professor.com.br ou http://seu-sistema.com/diario">');

        $this->campoRotulo(nome: 'lbl_url_whatsapp', campo: 'URL do WhatsApp', valor:
            '<input type="text" name="url_whatsapp" id="url_whatsapp" value="' . htmlspecialchars($this->url_whatsapp ?? '') . '" size="80" maxlength="500" style="width:100%;" placeholder="Ex: https://wa.me/5582999999999 ou https://api.whatsapp.com/send?phone=5582999999999">');
    }
    ...
    public function Editar()
    {
        ...
        // Processar logo: upload (base64) tem prioridade sobre URL
        $logoFinal = $this->ieducar_image ?? '';
        if (!empty($this->ieducar_image_upload) && str_starts_with($this->ieducar_image_upload, 'data:image/')) {
            $logoFinal = $this->ieducar_image_upload;
        }

        // Processar background: upload (base64) tem prioridade sobre URL
        $bgImageFinal = $this->ieducar_background_image ?? '';
        if (!empty($bgImageFinal) && !str_starts_with($bgImageFinal, 'data:image/')) {
            // Já era um valor salvo anteriormente — manter
        }
        $bgUrlFinal = $this->ieducar_background_image_url ?? '';

        // Checkbox: se não veio no POST, vem do hidden = 0
        $mostrarBotoes = ($this->mostrar_botoes_ajuda_login == '1' || $this->mostrar_botoes_ajuda_login === true) ? 1 : 0;
        ...
        $configuracoes = new clsPmieducarConfiguracoesGerais(ref_cod_instituicao: $ref_cod_instituicao, campos: [
            ...
            'ieducar_background_image'     => $bgImageFinal,
            'ieducar_background_image_url' => $bgUrlFinal,
            'url_diario_professor'         => $this->url_diario_professor ?? '',
            'url_whatsapp'                 => $this->url_whatsapp ?? '',
            'mostrar_botoes_ajuda_login'   => $mostrarBotoes,
        ]);
