...
return new class extends clsCadastro
{
    ...
    public function Inicializar()
    {
        ...
        $obj_permissoes->permissao_cadastra(int_processo_ap: 561, int_idpes_usuario: $this->pessoa_logada, int_soma_nivel_acesso: 7, str_pagina_redirecionar: 'educar_fornecedores.php');
        ...
        if (is_numeric($this->cod_escola)) {
            ...
            if ($registro === false) {
                throw new HttpResponseException(
                    new RedirectResponse('educar_fornecedores.php')
                );
            }
        ...
        $this->url_cancelar = ($retorno == 'Editar') ? "educar_escola_det.php?cod_escola={$registro['cod_escola']}" : 'educar_fornecedores.php';
    ...
    private function pessoaJuridicaContemEscola($pessoaj_id)
    {
        ...
        if (is_array($escola) && count($escola) > 0) {
            ...
            if (is_array($current) &&
                ...
                $this->mensagem = "Escola criada, para<a href=\"educar_fornecedore_cadastro.php?cod_escola={$current['cod_escola']}\"> editar clique aqui.</a>";
    ...
    public function Gerar()
    {
        ...
        if ($this->pesquisaPessoaJuridica) {
            ...
            $this->array_botao_url_script = ['obj = document.getElementById(\'pessoaj_idpes\');if(obj.value != \'\' ) {
                document.getElementById(\'tipoacao\').value = \'\'; acao(); } else { acao(); }', 'go(\'educar_fornecedores.php\');'];
        } else {
            $obj_permissoes = new clsPermissoes;
            $this->fexcluir = is_numeric($this->cod_escola) && $obj_permissoes->permissao_excluir(int_processo_ap: 561, int_idpes_usuario: $this->pessoa_logada, int_soma_nivel_acesso: 3);
            ...
            $this->breadcrumb(currentPage: 'Escola', breadcrumbs: ['educar_index.php' => 'Escola']);
            $this->url_cancelar = (!empty($this->cod_escola)) ? "educar_escola_det.php?cod_escola={$this->cod_escola}" : 'educar_fornecedores.php';
            $this->nome_url_cancelar = 'Cancelar';
        }
    }
    ...
    {
        $obj_permissoes = new clsPermissoes;
        $obj_permissoes->permissao_cadastra(int_processo_ap: 561, int_idpes_usuario: $this->pessoa_logada, int_soma_nivel_acesso: 3, str_pagina_redirecionar: 'educar_fornecedores.php');
        $this->pesquisaPessoaJuridica = false;
